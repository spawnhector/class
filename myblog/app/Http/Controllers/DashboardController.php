...
class DashboardController extends Controller
{
    ...
    public static function count_tags($tag)
    {
        $check_post = Post::all();
        $post_with_simtags = "";
        foreach ($check_post as $this_post ) {
            $check_if = explode(',',$this_post->meta_keywords);
            foreach ($check_if as $keyword ) {
                if ($keyword == $tag) {
                    $post_with_simtags .= $this_post->id." ";
                }
            }
        }
        return $post_with_simtags;
    }
    ...
    public function show()
    {
        ...
        for ($i=0; $i < count($get_trends_key); $i++) {
            ...
            foreach ($get_trends_post as $trends_post ) {
                ...
                $this_author = User::where('id','=',$trends_post->author_id)->get();
                $auth_name = "";
                foreach ($this_author as $authorrr ) {
                    $auth_name = $authorrr->name;
                }
                // dd($auth_name);
                $popular1 .= ''.$split.'<a class="img-link"><img class="s-img" src="http://localhost/xampp/myblog/storage/app/public/' . $trends_post->image . '"><span class="popular-link">'.$trends_post->title.'<p class="popular-link-icon"><div>Author: @'.$auth_name.'</div><div class="stats2"><span>Like: '.count($check_likes).'</span> . <span>Comment: '.count($check_comments).'</span> . <span>View: '.count($check_views).'</span></div></p></span></a>';
            }
        }
        ...
        $hidden_pop = "
        <button hidden id='show_pop' onclick='get_popular(".$popular_count.")' value='".$popular1."' ></button>
        ";
        $post_id1 = "
        <button hidden id='post-id' value=''></button>
        ";
        // dd($popular1);
    ...
    public function show_post($id, $is)
    {
        ...
        foreach ($this_post as $check_tag) {
            ...
            foreach ($tags_get as $get_tags ) {  
                $array_to_check = explode(" ", $this->count_tags($get_tags));
                $array_to_check_recheck = [];
                foreach ($array_to_check as $to_check) {
                    if ($to_check != "") {
                        array_push($array_to_check_recheck,$to_check);
                    }
                }
                $popover_content_unique = array_unique($array_to_check_recheck);
                $get_tag_post_id = "";
                foreach ($popover_content_unique as $unique) {
                    $get_tag_post_id .= $unique." "; 
                }
                $popover_content = count($popover_content_unique)." Post with this Tag.";
                $tags .= 
                "
                <a data-toggle='popover' title='#".$get_tags."' class='tags' onmouseover='search_tags()'  data-tag='".$popover_content."' data-view='VIEW' data-post_id='".$get_tag_post_id."'>#".$get_tags."</a>
                ";
            }
        }
        // dd($array_to_check_recheck);
        $popular = '';
        $split ="";
    ...
    public function show_post_comment($id,$ig,$is)
    {
        ...
        foreach ($get_comment as $comm ) {
            ...
            $replys1 = Reply::orderBy('created_at', 'desc')->where('comment_id', '=', $comm->id)->get();
            $reply_inc = count($get_comment) + 1;
        ...
        $store->content = $is;
        $store->image = Auth::User()->avatar;
        $store->slug =  Str::of($is.$reply_inc)->slug('-');
        $store->comment_id = $ig;
        $store->save();
        $comment = Comment::orderBy('created_at', 'desc')->where('post_id', '=', $id)->get();
        $comment_count = count($comment);
    ...
    public function get_post_comment($id,$is)
    {

        
        $datas1 = Post::get()->where('id', '=', $id);
        $post_author1 = "";
        foreach ($datas1 as $post_data ) {
            $post_author_data = User::where('id','=',$post_data->author_id)->get();
            foreach ($post_author_data as $author_data ) {
                $post_author1 = $author_data->name;
            }
        }
        $comment = Comment::orderBy('created_at', 'desc')->where('post_id', '=', $id)->get();
        $comment_count = count($comment);
        $comment_count_inc = count($comment) + 1;
        $store = new Comment;
        $store->user_id = Auth::user()->id;
        $store->title = $titles = Auth::user()->name." coment on ".$post_author1." post";
        $store->content = $is;
        $store->user_avatar = Auth::User()->avatar;
        $store->slug =  Str::of($is.$id.$comment_count_inc)->slug('-');
        $store->post_id =  $id;
        $store->save();
    ...
    public function show_searchpost($id,$is)
    {
        return $this->show_post($id,$is);
    }
    ...
    public function chat_now($id, $is)
    {
        ...
        $show_chat = "<div class='chat-spacer'></div>";
    ...
    public function get_chat_now($id)
    {
        ...
        foreach ($get_chat as $chats) {
            ...
            if ($chats->user_id == Auth::user()->id) {
                ...
                $show_chat .=
                    "
                <div class='me'><div class='me_text'><img class='chat_image' src='http://localhost/xampp/myblog/storage/app/public/" . Auth::user()->avatar . "' alt=''><p class='me_chat_txt'>" . $chats->message . "</p><span class='me_chat_datd'>" . $chat_date . "</span></div></div>
                ";
            } else {
                $chat_date = $chats->created_at->diffForHumans();
                $image = "";
                ...
                $show_chat .=
                    "
                <div class='you'><div class='you_text'><img class='chat_image_you' src='http://localhost/xampp/myblog/storage/app/public/" . $image . "' alt=''><p class='you_chat_txt'>" . $chats->message . "</p><span class='you_chat_datd'>" . $chat_date . "</span></div></div>
                ";
            }
        }
        return response(
            ...
            if ($request->title == "" && $request->content == "") {
                ...
                if ($image = $request->file('Image')->store('posts', 'public')) {
                    ...
                    if($request->meta_keywords == ""){
                        $get_tags = explode(' ',$request->title);
                        $count_tags = 0;
                        $count_tags1 = count($get_tags);
                        $separeter = ",";
                        foreach ($get_tags as $tags ) {
                            if ($count_tags == $count_tags1-1) {
                                $separeter = "";
                                $count_tags = $count_tags1-1;
                            } else {
                                $separeter = ",";
                                ++$count_tags;
                            }
                            $post_tags .= $tags.$separeter;
                        }
                    }else{
                        $get_tags1 = explode(' ',$request->title);
                        $get_tags2 = explode(' ',$request->meta_keywords);
                        $count_tags = 0;
                        $count_tags1 = count($get_tags2);
                        $separeter = ",";
                        foreach ($get_tags1 as $tags1 ) {
                            $post_tags .= $tags1.$separeter;
                        }
                        foreach ($get_tags2 as $tags2 ) {
                            if ($count_tags == $count_tags1-1) {
                                $separeter = "";
                                $count_tags = $count_tags1-1;
                            } else {
                            ...
                            $post_tags .= $tags2.$separeter;
                        }
                    }
                    $save->meta_keywords = $post_tags;
    ...
    public function edit_post(Request $request,$ir){
        $auth_user = Auth::user();
        $post_id = Auth::user()->id;
        // dd($ir);
        $post = Post::orderBy('created_at', 'desc')->get()->where('author_id', '=', $post_id);
        $get_post = Post::orderBy('created_at', 'desc')->get()->where('id', '=', $ir);
        $titlee = "";
        $content = "";
        $imagee = "";
        $keyword = "";
        foreach ($get_post as $postss ) {
            $titlee = $postss->title;
            $content = $postss->body;
            $imagee = $postss->image;
            $keywords = $postss->meta_keywords;
            $get_keyword = explode(',',$keywords);
            foreach ($get_keyword as $getkey ) {
                $keyword .= $getkey." ";
            }
        }
    ...
    public function search($id)
    {
        ...
        if (count($search_post) > 0) {
            ...
            foreach ($search_post as $post) {
                ...
                $datas .= "
                <a href='http://127.0.0.1:8000/dashboard/show-searchpost/" . $post->id . "/" . $post->author_id . "'>
    ...
    public function datecreate($date) { if ($date=='1' ) { return 'JAN' ; } elseif ($date=='2' ) { return 'FEB' ; } elseif
    ($date=='3' ) { return 'MAR' ; } elseif ($date=='4' ) { return 'APR' ; } elseif ($date=='5' ) { return 'MAY' ; } elseif
    ($date=='6' ) { return 'JUN' ; } elseif ($date=='7' ) { return 'JUL' ; } elseif ($date=='8' ) { return 'AUG' ; } elseif
    ($date=='9' ) { return 'SEP' ; } elseif ($date=='10' ) { return 'OCT' ; } elseif ($date=='11' ) { return 'NOV' ; } elseif
    ($date=='12' ) { return 'DEC' ; } }

    public function message(){

        $get_message = Message::orderBy('created_at', 'desc')->where('to','=',Auth::user()->id)->get();
        $inbox = "";
        $count_checkbox = 0;
        foreach ($get_message as $user_message ) {
            $from = User::where('id','=',$user_message->from)->get();
            $get_sender = "";
            foreach ($from as $sender ) {
                if ($sender->name == Auth::user()->name) {
                    $get_sender = "me";
                } else {
                    $get_sender = $sender->name;
                }
                
            }
            $datestrain = explode(' ', $user_message->created_at);
            $date = explode('-', $datestrain[0]);
            $new_date = $this->datecreate($date[1]);
            $unread = "";
            if ($user_message->status == "unread") {
                $unread = "unread";
            }
            // dd($new_date);
            $inbox .= "
            <li class='blue-dot ".$unread."' onclick='get_message(".$user_message->id.",".$user_message->from.")'>
                <div class='col col-1'><span class='dot'></span>
                    <div class='checkbox-wrapper'>
                        <input name='checkbox".$count_checkbox."' type='checkbox' id=''>
                        <label for='chk13' class='toggle'></label>
                    </div>
                    <p class='title'>".$get_sender."</p><span
                        class='star-toggle glyphicon glyphicon-star-empty'></span>
                </div>
                <div class='col col-2'>
                    <div class='subject'>".$user_message->subject."</div>
                    <div class='date'>".$new_date." ".$date[2]."</div>
                </div>
            </li>";
            ++$count_checkbox;
        }

        $get_sent = Message::orderBy('created_at', 'desc')->where('from','=',Auth::user()->id)->get();
        $sent = "";
        foreach ($get_sent as $user_sent ) {
            $from = User::where('id','=',$user_sent->from)->get();
            $get_sender = "";
            foreach ($from as $sender ) {
                if ($sender->name == Auth::user()->name) {
                    $get_sender = "me";
            ...
            $datestrain = explode(' ', $user_sent->created_at);
            $date = explode('-', $datestrain[0]);
            $new_date = $this->datecreate($date[1]);
            $unread = "";
            if ($user_sent->status == "unread") {
                $unread = "unread";
            }
            // dd($new_date);
            $sent .= "
            <li class='blue-dot ".$unread."' onclick='get_message(".$user_sent->id.",".$user_sent->from.")'>
                <div class='col col-1'><span class='dot'></span>
                    <div class='checkbox-wrapper'>
                ...
                <div class='col col-2'>
                    <div class='subject'>".$user_sent->subject."</div>
                    <div class='date'>".$new_date." ".$date[2]."</div>
                </div>
            </li>";
        }
        return view('message',[
            'inboxs'=>$inbox,
            'inbox'=> count($get_message),
            'sent'=>$sent,
            'sent_count'=>count($get_sent),
        ]);
    }

    public function store_message(Request $request){
        $check_message = $request->validate([
            'to' => ['required'],
            'subj' => ['required'],
            'mcontent' => ['required'],
        ]);
        $recepient = User::where('name','=',$request->to)->get();
        $to_id = 0;
        foreach ($recepient as $recp ) {
            $to_id = $recp->id;
        }
        // dd($request);
        $store_message = new Message;
        $store_message->to = $to_id;
        $store_message->from = Auth::user()->id;
        $store_message->subject = $request->subj;
        $store_message->slug = $slug =Str::of($request->subj)->slug('-');
        $store_message->status = "unread";
        $store_message->save();

        $get_messagee = Message::where('slug','=',$slug)->get();
        $messages_id = 0;
        foreach ($get_messagee as $user_message ) {
            $messages_id = $user_message->id;
        };
        $store_message_chat = new Message_chat;
        $store_message_chat->to = $to_id;
        $store_message_chat->from = Auth::user()->id;
        $store_message_chat->message_id = $messages_id;
        $store_message_chat->content = $request->mcontent;
        $store_message_chat->save();
        
        $users = User::find($to_id);
        Notification::send($users, new messageNotification(Auth::user()->name, Auth::user()->avatar));

        return redirect('dashboard/message')->with([
    ...
    public function reply_message(Request $request){
        ...
        $store_message_chat = new Message_chat;
        $store_message_chat->to = $request->to;
        $store_message_chat->from = Auth::user()->id;
        $store_message_chat->message_id = $request->message_id;
        $store_message_chat->content = $request->mcontent;
        $store_message_chat->save();

        return redirect('dashboard/message')->with([
            'message'    => __('Reply success'),
            'alert-type' => 'success',
        ]);
        // dd($request);
    }
    public function delete_message_chat($id){
        $get_message_chat = Message_chat::find($id);
        $get_message_chat->delete();
        
        return redirect('dashboard/message')->with([
            'message'    => __('Message chat as been deleted'),
            'alert-type' => 'success',
        ]);
        // dd($id);
    }
    ...
    public function get_message($id){
        $messsage_chat_li = "";
        $get_message_chat = Message_chat::where('message_id','=',$id)->get();
        foreach ($get_message_chat as $message_chat) {
            $datestrain = explode(' ', $message_chat->created_at);
            $date = explode('-', $datestrain[0]);
            $new_date = $this->datecreate($date[1]);
            $send_recieve = "";
            $sender = "";
            if ($message_chat->from == Auth::user()->id) {
                $send_recieve = "sent";
                $sender_name = "";
                if ($message_chat->to == Auth::user()->id) {
                    $sender_name = "You";
                } else {
                    $user_get = User::where('id','=',$message_chat->to)->get();
                    foreach ($user_get as $get_user ) {
                ...
                "
                <div class='left'>You
                    <div class='arrow'></div>". $sender_name."
                </div>
                ";
            } else {
                $send_recieve = "received";
                $sender_name = "";
                $user_get = User::where('id','=',$message_chat->from)->get();
                foreach ($user_get as $get_user ) {
                    $sender_name = $get_user->name;
                }
                $sender = 
                "
                <div class='left'>". $sender_name."
            ...
            $messsage_chat_li .= 
            "
            <li class='".$send_recieve."'>
                <div class='details'>
                    ".$sender."
                    <div class='right'>".$date[2]." ".$new_date."</div>
                </div>
                <div class='message'>
                    <p>".$message_chat->content."</p>
                </div>
                <div class='tool-box'><a href='#' class='circle-icon small glyphicon glyphicon-share-alt'></a><a
                        href='message/chat/delete/".$message_chat->id."' class='circle-icon small red-hover glyphicon glyphicon-remove'></a><a href='#'
                        class='circle-icon small red-hover glyphicon glyphicon-flag'></a></div>
            </li>
            ";
        }


        $get_message_chat_main = Message::where('id','=',$id)->get();
        $to = "";
        $from = "";
        foreach ($get_message_chat_main as $chat_main ) {
            if ($chat_main->to == Auth::user()->id) {
                $to  = "You";
            } else {
                $user_get = User::where('id','=',$chat_main->to)->get();
                foreach ($user_get as $get_user ) {
                    $to = $get_user->name;
                }
            }
            if ($chat_main->from == Auth::user()->id) {
                $from  = "You";
            } else {
                $user_get = User::where('id','=',$chat_main->from)->get();
                foreach ($user_get as $get_user ) {
                    $from = $get_user->name;
                }
            }
        ...
        $datestrain = explode(' ', $chat_main->created_at);
        $date = explode('-', $datestrain[0]);
        $new_date = $this->datecreate($date[1]);
        $messsage_chat = 
        "
        <div class='header'>
            ...
            <a
                    class='icon glyphicon glyphicon-chevron-left trigger-message-close'></a>Message<span
                    class='grey'>(".count($get_message_chat).")</span></h1>
            <p class='page-ti'>From <a href='#'>".$from."</a> to <a href='#'>".$to."</a>, started on <a href='#'>".$date[2]." ".$new_date."</a>
                </p>
                <div class='message-reply' id='message-reply' style='display:none;'>
                
                </div>
                <a data-toggle='tooltip' title='Reply' onclick='replym()'><svg enable-background='new 0 0 24 24' height='30' viewBox='0 0 24 24' width='30' xmlns='http://www.w3.org/2000/svg' style='position: absolute;right: -68px;top: 23px;' ><path d='m23.5 23c-.169 0-.331-.086-.424-.235-1.473-2.357-4.013-3.765-6.792-3.765h-.284v2.5c0 .202-.122.385-.309.462-.187.08-.401.035-.545-.108l-5-5c-.195-.195-.195-.512 0-.707l5-5c.144-.144.357-.186.545-.108.187.076.309.259.309.461v2.515c4.455.26 8 3.966 8 8.485 0 .223-.148.419-.362.48-.046.014-.092.02-.138.02zm-8-5h.784c2.506 0 4.838 1.017 6.519 2.786-.778-3.312-3.757-5.786-7.303-5.786-.276 0-.5-.224-.5-.5v-1.793l-3.793 3.793 3.793 3.793v-1.793c0-.276.224-.5.5-.5z'/><path d='m9.5 15h-7.5c-1.103 0-2-.897-2-2v-10c0-1.103.897-2 2-2h18c1.103 0 2 .897 2 2v9.5c0 .276-.224.5-.5.5s-.5-.224-.5-.5v-9.5c0-.551-.449-1-1-1h-18c-.551 0-1 .449-1 1v10c0 .551.449 1 1 1h7.5c.276 0 .5.224.5.5s-.224.5-.5.5z'/><path d='m11 9.578c-.366 0-.732-.083-1.066-.248l-9.656-4.782c-.248-.123-.349-.423-.227-.67.123-.248.422-.349.67-.227l9.655 4.782c.391.193.855.193 1.246 0l9.655-4.782c.248-.123.546-.021.67.227.123.247.021.547-.227.67l-9.654 4.782c-.334.165-.7.248-1.066.248z'/></svg></a>
        </div>
