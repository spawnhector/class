...
// require 'C:\xampp\htdocs\xampp\test opensource\cryptoPub\app\Http\Livewire\User\faucet\faucetinabox\libs/functions.php';
...
require_once("".faucetbox_path()."script/common.php");
// require __DIR__ . '/script/common.php';
// dd($dbtable_prefix);

if (array_key_exists("p", $_POST) && in_array($_POST["p"], ["admin", "password-reset"])) {
   redirect()->back();
    // header("Location: adminfaucet?p={$_POST["p"]}");
    // die("Please wait...");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // require __DIR__ . '/libs/adminlog.php';
...
// Get template
// dd($faucet_settings_array['template']);
$template = $faucet_settings_array['template'];
if (!file_exists("".faucetbox_path()."templates/{$template}/index.blade.php")) {
    $templates = glob("templates/*");
    // dd($templates);
    if ($templates) {
        $template = substr($templates[0], strlen("templates/"));
    } else {
        die(str_replace('<:: content ::>', "<div class='alert alert-danger' role='alert'>No templates found!</div>",''));
    }
}
...
if (array_key_exists('cc', $_POST) && in_array($_POST['cc'], $captcha['available'])) {
    $captcha['selected'] = $captcha['available'][array_search($_POST['cc'], $captcha['available'])];
...
$data["eligible"] = checkTimeForIP(getIP(), $time_left,$data);
if (($data["eligible"]) && (!empty($data["address"]))) {
    $data["eligible"] = checkTimeForAddress(trim($data["address"]), $time_left,$data);
}
...
if (array_key_exists('address', $_POST) && $data['enabled'] && $data['eligible']) {
    ...
    $error = getClaimError($address,$data,$captcha,$security_settings);
    if ($error) {
        ...
        if ($ret['success']) {
            ...
            if (array_key_exists('r', $_POST) && trim($_POST['r']) != $address) {
                if (($faucet_settings_array['service']=='expresscrypto') && (stripos($_POST['r'], 'EC-')===false)) {
                    unset($_POST['r']);
                }
            }
            ...
            if (array_key_exists('r', $_POST) && trim($_POST['r']) != $address) {
                ...
                $q->execute(array(textSanitize(trim($_POST['r']))));
                ...
                $q->execute(array(trim($_POST['address']), textSanitize(trim($_POST['r']))));
...
// dd(file_get_contents("C:/xampp/htdocs/xampp/test opensource/cryptoPub/server.php"));
preg_match_all('/\$data\[([\'"])(custom_(?:(?!\1).)*)\1\]/', file_get_contents("C:/xampp/htdocs/xampp/test opensource/cryptoPub/resources/views/faucet/faucetinabox/templates/$template/index.blade.php"), $matches);
...
require_once('templates/'.$template.'/index.blade.php');
