...
$db_updates = array(
    1 => array(
        ...
        "CREATE TABLE if not exists `".$dbtable_prefix."Cryptopub_Faucet` (
    ...
    104 => array(
        "INSERT IGNORE INTO `".$dbtable_prefix."Settings` (`name`, `value`) VALUES ('service', 'expresscrypto');"
        ),
    107 => array(
...
if (intval($version) >= intval($dbversion)) {
    ...
    $q->execute(array('0'));
    // reload settings
    $faucet_settings_array = fb_load_settings($sql);
    if ($initial_install) {
        // dd('here');
        // set password
        require_once("".faucetbox_path()."script/admin_templates.php");
        $password = setNewPass();
