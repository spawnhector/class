...
function fb_load_settings() {
    ...
    if (isset($_POST['cryptopubfaucet'])) {
        $faucet_settings_array['service'] = ddecrypt($_POST['faucet_service']);
        $faucet_settings_array['name'] = ddecrypt($_POST['faucet_name']);
        $faucet_settings_array['apikey'] = ddecrypt($_POST['faucet_apikey']);
        $faucet_settings_array['currency'] = ddecrypt($_POST['faucet_currency']);
        $faucet_settings_array['rewards'] = ddecrypt($_POST['faucet_rewards']);
        $faucet_settings_array['short'] = ddecrypt($_POST['faucet_short']);
        $faucet_settings_array['timer'] = ddecrypt($_POST['faucet_timer']);
        $faucet_settings_array['referral'] = ddecrypt($_POST['faucet_referral']);
        // dd(ddecrypt($_POST['faucet_referral']));
    }
...
function checkRevProxyIp($file, $remote_addr) {
    require_once(''.faucetbox_path().'libs/http-foundation/IpUtils.php');
...
function detectRevProxyProvider() {
    ...
    if(checkRevProxyIp("".faucetbox_path()."libs/ips/cloudflare.txt", $remote_addr)) {
        return "CloudFlare";
    } elseif(checkRevProxyIp("".faucetbox_path()."libs/ips/incapsula.txt", $remote_addr)) {
        return "Incapsula";
    }
    return "none";
}
...
function getIP() {
    ...
    $ip = null;
    $server = isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : $_SERVER['REMOTE_ADDR'];
    $remote_addr = $_SERVER['REMOTE_ADDR'];
    if (($remote_addr==$server) && (isset($_SERVER['HTTP_X_REAL_IP']))) {
        $remote_addr = $_SERVER['HTTP_X_REAL_IP'];
    }
...
function regenerate_csrf_token() {
    ...
    $_SESSION['csrftoken'.session_prefix()] = csrf_token();
}

function get_csrf_token() {
    return '<input type="hidden" name="_token" value="'. $_SESSION['csrftoken'.session_prefix()]. '">';
}
...
function setNewPass($password = '') {
    ...
    // ultimate - bugfix
    $hash = crypt($password, md5_file(''.faucetbox_path().'/config.php'));
...
function checkTimeForIP($ip, &$time_left = NULL,$data) {
    $dbtable_prefix = prefix();
    $sql = sql();
...
function checkTimeForAddress($address, &$time_left = NULL,$data) {
    $dbtable_prefix = prefix();
    $sql = sql();
    $faucet_settings_array = fb_load_settings();
...
function checkAddressBlacklist($address,$security_settings) {
    // dd($address);
    return !in_array($address, $security_settings["address_ban_list"]);
}

function checkIPIsWhitelisted($security_settings) {
    $ip = ip2long(getIP());
...
function checkIPBlacklist($security_settings) {
    $ip = ip2long(getIP());
    if ($ip) { // only ipv4 supported here
...
function checkProxyData($hostnames_array,$security_settings) {
...
function checkCaptcha($data,$captcha) {
    ...
    switch ($captcha['selected']) {
        ...
        break;
        case 'SolveMedia':
            require_once(''.faucetbox_path().'libs/solvemedialib.php');
        ...
        break;
        case 'hCaptcha':
            require_once(''.faucetbox_path().'libs/hcaptcha.php');
            $hcaptchaobj = new hcaptcha();
            return $hcaptchaobj->checkResult($_POST['h-captcha-response'],$faucet_settings_array);
        break;
        case 'RainCaptcha':
...
function getClaimError($address,$data,$captcha,$security_settings) {
    global $dbtable_shortlink_pool_prefix;
    $dbtable_prefix = prefix();
    $sql = sql();
    $faucet_settings_array = fb_load_settings();
    ...
    if (!checkTimeForAddress($address, $time_left,$data)) {
        return 'You have to wait '.$time_left.' minutes';
    }
    if (!checkTimeForIP(getIP(), $time_left,$data)) {
        return 'You have to wait '.$time_left.' minutes';
    }
    ...
    # AntiBotLinks START
    $antibotlinkss = antibotlink();
    // dd(antibotlink());
    if (!$antibotlinkss->is_valid()) {
    ...
    if (!checkAddressBlacklist($address,$security_settings)) {
    ...
    if (!empty($_POST['r'])) {
      if (!checkAddressBlacklist($_POST['r'],$security_settings)) {
    ...
    $q->execute(array(trim($address)));
    if ($ref = $q->fetch()) {
        if (!checkAddressBlacklist(trim($ref[0]),$security_settings)) {
            nastyhosts_log('deny', 'Your ref *coin address has been blacklisted.', array());
            return 'Your ref *coin address has been blacklisted.';
        }
    }
    //
    if(!checkIPIsWhitelisted($security_settings)) {
        if (!checkIPBlacklist($security_settings)) {
        ...
        if (count($proxycheck_array)>0) {
            ...
            if ($proxycheck_row = $q->fetch(PDO::FETCH_ASSOC)) {
                ...
                $proxyDataResult = checkProxyData($hostnames_array,$security_settings);
    ...
    if (($force) || ($faucet_settings_array['reward_in_USD_last_check']<time()-3600)) {
        if ($ch = curl_init()) {
            ...
            switch ($faucet_settings_array['currency']) {
                case 'ADA':
                    $currency_coingecko = 'cardano';
                break;
                case 'BCH':
                    $currency_coingecko = 'bitcoin-cash';
                break;
                case 'BCN':
                    $currency_coingecko = 'bytecoin';
                break;
                case 'BLK':
                    $currency_coingecko = 'blackcoin';
                break;
                case 'BSV':
                    $currency_coingecko = 'bitcoin-cash-sv';
                break;
                case 'BTC':
                    $currency_coingecko = 'bitcoin';
                break;
                case 'BTG':
                    $currency_coingecko = 'bitcoin-gold';
                break;
                case 'BTX':
                    $currency_coingecko = 'bitcore';
                break;
                case 'CURE':
                    $currency_coingecko = 'curecoin';
                break;
                case 'DASH':
                    $currency_coingecko = 'dash';
                break;
                case 'DCR':
                    $currency_coingecko = 'decred';
                break;
                case 'DGB':
                ...
                break;
                case 'EOS':
                    $currency_coingecko = 'eos';
                break;
                case 'ETC':
                    $currency_coingecko = 'ethereum-classic';
                break;
                case 'ETH':
                    $currency_coingecko = 'ethereum';
                break;
                case 'GAME':
                    $currency_coingecko = 'gamecredits';
                break;
                case 'HORA':
                    $currency_coingecko = 'hora';
                break;
                case 'LSK':
                    $currency_coingecko = 'lisk';
                break;
                case 'LTC':
                    $currency_coingecko = 'litecoin';
                break;
                case 'MAID':
                    $currency_coingecko = 'maidsafecoin';
                break;
                case 'MUE':
                    $currency_coingecko = 'monetaryunit';
                break;
                case 'NEO':
                    $currency_coingecko = 'neo';
                break;
                case 'OMNI':
                    $currency_coingecko = 'omni';
                break;
                case 'POT':
                ...
                break;
                case 'QTUM':
                    $currency_coingecko = 'qtum';
                break;
                case 'RDD':
                    $currency_coingecko = 'reddcoin';
                break;
                case 'STRAT':
                    $currency_coingecko = 'stratis';
                break;
                case 'TRX':
                    $currency_coingecko = 'tron';
                break;
                case 'WAVES':
                    $currency_coingecko = 'waves';
                break;
                case 'VTC':
                    $currency_coingecko = 'vertcoin';
                break;
                case 'XMR':
                ...
                break;
                case 'XRP':
                    $currency_coingecko = 'ripple';
                break;
                case 'XTZ':
                    $currency_coingecko = 'tezos';
                break;
                case 'XZC':
                    $currency_coingecko = 'zcoin';
                break;
                case 'ZEC':
