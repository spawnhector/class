...
class WalCrypt
{
    protected $api_key;
    protected $currency;
    protected $api_base = "https://walcrypt.com/api/v1/";
    ...
    public function send($to, $amount, $userip, $referral = 'false') {
        ...
        if (is_array($r) && array_key_exists("status", $r) && $r["status"] == 200) {
            return array(
                'success' => true,
                'message' => 'Payment sent to you using WalCrypt.com',
                'html' => '<div class="alert alert-success">' . htmlspecialchars($amount) . ' satoshi was sent to you on WalCrypt.com.</div>',
                'html_coin' => '<div class="alert alert-success">' . htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' '.$this->currency.' was sent to you WalCrypt.com.</div>',
                'balance' => (int)$r["balance"],
                'balance_bitcoin' => $r["balance_decimal"],
