...
if (session_id()) {
    // $_SESSION['csrftoken'.$session_prefix]='';
    if (empty($_SESSION['csrftoken'.$session_prefix])) {
        regenerate_csrf_token();
    }
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // dd(session());
        if (empty($_POST["_token"]) || $_SESSION['csrftoken'.$session_prefix] != $_POST["_token"]) {
...
# NH START
require_once(''.faucetbox_path().'libs/nh.php');
$nh=new nh();
$nh->admin_config_top();
# NH END
...
# ADMINLOG START
require_once(''.faucetbox_path().'libs/adminlog.php');
...
if (array_key_exists('p', $_POST) && $_POST['p'] == 'logout') {
    $_SESSION = array();
...
// dd($_POST);
if (array_key_exists('p', $_POST) && $_POST['p'] == 'password-reset') {
    $error = "";
    if (array_key_exists('dbpass', $_POST)) {
        // dd($faucet_settings_array);
        ...
        if ($captcha->isValid($user_captcha)) {
            if ($_POST['dbpass'] == $dbpass) {
                ...
                $page = str_replace('<:: content ::>', $pass_template, $master_template);
                $page = str_replace('<:: password ::>', $password, $page);
                $page = str_replace("<:: admin_continue ::>", redirectPr('faucet','Click here to continue',
                "
...
if (array_key_exists('session_check', $_POST)) {
    // dd($_POST['session_check']);
...
if (isset($_SESSION['logged_in'.$session_prefix])) { // logged in to admin page
    ...
    if (
            array_key_exists("update_status", $_POST) &&
            in_array($_POST["update_status"], ["success", "fail"])
            ) {
        if ($_POST["update_status"] == "success") {
    ...
    if (array_key_exists('save_settings', $_POST)) {
        ...
        preg_match_all('/\$data\[([\'"])(custom_(?:(?!\1).)*)\1\]/', file_get_contents("".faucetbox_path()."templates/$template/index.blade.php"), $matches);
        ...
        if (!empty($_POST['apikey'])) {
            ...
            $q = $sql->prepare("SELECT * FROM ".$dbtable_prefix."cryptopub_faucet WHERE service = ? AND api_key = ?;");
            $q->execute(array($_POST['service'],$_POST['apikey']));
            if ($row = $q->fetch()) {
                $q = $sql->prepare("UPDATE ".$dbtable_prefix."cryptopub_faucet SET name = ?, currency = ?, rewards = ?, short = ?, timer = ?, referral = ? WHERE service = ? AND api_key = ?;");
                $q->execute(array($_POST['name'],$_POST['currency'],$_POST['rewards'],$_POST['short'],$_POST['timer'],$_POST['referral'],$_POST['service'],$_POST['apikey']));
            }else{
                $fl = $sql->prepare("INSERT INTO ".$dbtable_prefix."cryptopub_faucet (service, api_key, name, currency, rewards, short, timer, referral, status,last_used) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $fl->execute(array($_POST['service'],$_POST['apikey'],$_POST['name'],$_POST['currency'],$_POST['rewards'],$_POST['short'],$_POST['timer'],$_POST['referral'],'1',now()));
            }
...
# ADMINLOG END
    ...
    while ($row = $query->fetch()) {
        if ($row[0] == 'template') {
            // dd($row[1]);
            if (file_exists("".faucetbox_path()."templates/{$row[1]}/index.blade.php")) {
    ...
    $templates = '';
    foreach (glob("".faucetbox_path()."templates/*") as $template) {
        // dd($template);
    ...
    if (file_exists("".faucetbox_path()."templates/{$current_template}/setup.php")) {
        require_once("".faucetbox_path()."templates/{$current_template}/setup.php");
    ...
    $template_string = file_get_contents("".faucetbox_path()."templates/{$current_template}/index.blade.php");
    ...
    $new_files = [];
    foreach (new RecursiveIteratorIterator (new RecursiveDirectoryIterator (''.faucetbox_path().'templates')) as $file) {
    ...
    "
        ...
        <input type='hidden' name='p' value='admin'>
    "), $page);
    $page = str_replace("<:: faucetbox_path ::>", faucetbox_path(), $page);
    $page = str_replace("<:: admincheck ::>", 'http://localhost//xampp/test%20opensource/cryptoPub/resources/views/faucet/faucetinabox/admincheck.php', $page);
