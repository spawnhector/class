...
class makeFakeWebinars extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($teachers as $teacher) {
            $categories = Category::whereNotNull('parent_id')->get()->random(3);
            ...
            foreach ($categories as $category) {
                ...
                $filters = Filter::where('category_id', $category->id)->get()->random(2);

                foreach ($filters as $filter) {
                    $filterOptions = FilterOption::where('filter_id', $filter->id)->get()->random(3);
